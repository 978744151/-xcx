...
Page({
  ...
  onLoad: function (options) {
    ...
    this.setData({ access_token, uid, isFrist: true, tablesid, resHeight: `${res.windowHeight}px` });
  ...
  getData: function () {
    ...
    let myCart = wx.getStorageSync('shopInfo').myCart || [],
    ...
    console.log(myCart, total_dish_count, packvalue, total_price);

    Promise.all([WMAPI.getYuDianCartegory(parmas), WMAPI.getYuDianGoodsList(parmas)]).then((res) => {
      console.log(res);
    ...
    WMAPI.GetTremGo(parmas).then((res) => {
      console.log(res.data.data.limitcost, 8888)
      this.setData({
        limitconst: res.data.data.limitcost
      })
    })
  },
  ...
  goCreateOrder: function () { //跳转到创建订单页面
    ...
    if (this.data.total_dish_count > 0) { //购物车数量大于0时才让提交
      ...
      // wx.setStorageSync('packvalue', this.data.packvalue);
      var shopInfo = {
      ...
      this.closePullCart();
      wx.navigateTo({
        url: '/pages/createOrder_out/createOrder_out?tablesid=' + this.data.tablesid + '&storeid=' + app.shopId + '&isZT=true' + '&isJC=true'   //'&isZT=true' + '&isJC=true' 跳转携带自定义参数
      })
    }
  },
  closediv: function () {
  ...
  addNum(menuId, goodsid, tid) {
    ...
    let myCartSup = this.data.myCartSup,
      total_dish_count = this.data.total_dish_count,
      total_price = this.data.total_price,
      limitconst = this.data.limitconst,
      myCart = [],
    ...
    if (_specs) {
      ...
      total_price = (Number(total_price) + Number(_specs[tid[1]].marketprice) + Number(_specs[tid[1]].packvalue)).toFixed(2);
      specsName = _specs[tid[1]].specs_name
      console.log(specsName)
      marketprice = _specs[tid[1]].marketprice
    } else {
      packvalue = Number(packvalue) + Number(goodsList[menuId][goodsid].packvalue)
      total_price = (Number(total_price) + Number(curGoods.marketprice) + Number(goodsList[menuId][goodsid].packvalue)).toFixed(2);
      specsName = curGoods.specs_name
      marketprice = curGoods.marketprice
    }
    this.setData({
      need_price: (limitconst - total_price).toFixed(2)
    })
    if (tid instanceof Array) {
      tid = tid.join('_')
    }
    // console.log(curGoods)
    // console.log(myCartSup)
    // console.log(this.data.need_price,1) 
  ...
  minNum(menuId, goodsid, tid) {
    ...
    // console.log(this.data.need_price, 2) 
    if (myCartSup.hasOwnProperty(tid)) {
