...
Page({
  ...
  getData: function () {
    ...
    let myCart = wx.getStorageSync('myCart') || [],
      ...
      packvalue = wx.getStorageSync('packvalue') || 0,
  ...
  addTypeCart: function (e) { //多规格添加
    // console.log(e)
    // console.log(this.data.goodsHasAttr._specs)
  ...
  goCreateOrder: function () { //跳转到创建订单页面
    ...
    if (this.data.total_dish_count > 0) { //购物车数量大于0时才让提交
      ...
      this.closePullCart();
      wx.navigateTo({
        url: '/pages/createOrder/createOrder?tablesid=' + this.data.tablesid + '&storeid=' + app.shopId + '&isZT=true' + '&isJC=true'   //'&isZT=true' + '&isJC=true' 跳转携带自定义参数
      })
    }
  },
  closediv: function () {
  ...
  addNum(menuId, goodsid, tid, ) {
    ...
    if (_specs) {
      tid = tid.split('_')
      var sepcPackvalue = Number(_specs[tid[1]].packvalue) || 0;
      packvalue = Number(packvalue) + sepcPackvalue
      total_price = (Number(total_price) + Number(_specs[tid[1]].marketprice) + sepcPackvalue).toFixed(2);
      specsName = _specs[tid[1]].specs_name
      console.log(packvalue, specsName)
      marketprice = _specs[tid[1]].marketprice
    } else {
      var normalPackvalue = Number(goodsList[menuId][goodsid].packvalue) || 0;
      packvalue = Number(packvalue) + normalPackvalue
      // console.log(packvalue)
      total_price = (Number(total_price) + Number(curGoods.marketprice) + normalPackvalue).toFixed(2);
  ...
  minNum(menuId, goodsid, tid) {
    ...
    console.log(goodsList[menuId][goodsid].packvalue, packvalue)
